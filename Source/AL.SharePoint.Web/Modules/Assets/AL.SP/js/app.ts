...
// http://stackoverflow.com/questions/16204142/how-do-i-define-a-knockout-binding-handler-in-typescript
...
interface KnockoutBindingHandlers {
    renderUser: KnockoutBindingHandler;
    clientPeoplePicker: any;
    webPartId: KnockoutBindingHandler;
    submitOnEnter: KnockoutBindingHandler;
...
declare var ProcessImn: temp;
declare var RenderUserFieldWorker: temp;
declare var SPClientPeoplePicker_InitStandaloneControlWrapper: temp;
declare var ExecuteOrDelayUntilScriptLoaded: temp;
...
ko.bindingHandlers.renderUser = {
    ...
    init: function (element, valueAccessor) {
        ...
        var ru:any = ko.bindingHandlers.renderUser;
        if (ru.context == null) {
            var clientContext = SP.ClientContext.get_current();
            ru.context = clientContext;
    ...
    renderPresence: function (user, fieldSchemaOverride) {
        // ??
        var renderCtx = <SPClientTemplates.RenderContext>(new ContextInfo());
        //var renderCtx = <SPClientTemplates.RenderContext>(GetCurrentCtx());
        renderCtx.Templates = {};
...
function pageInEditMode() {
    var inEditMode = null;
    if ((<any>document.forms[MSOWebPartPageFormName]).MSOLayout_InDesignMode) {
        inEditMode = (<any>document.forms[MSOWebPartPageFormName]).MSOLayout_InDesignMode.value;
    }
    var wikiInEditMode = null;
    if ((<any>document.forms[MSOWebPartPageFormName])._wikiPageMode) {
        wikiInEditMode = (<any>document.forms[MSOWebPartPageFormName])._wikiPageMode.value;
    }
...
function loadMembersWebPart(initUsers) {
    var partId = "ElementKOPeoplePicker" + curId++;
